...
namespace ParallelSurf
{
	...
	public:
		...
		*/
		...
		// setup the integral image
		void init(cv::Mat _Img, unsigned int iWidth, unsigned int iHeight);
		...
		static bool getDoRandomInit()
		{
			return DoRandomInit;
		}

	public:
		// pixel data of the image(存储图像数据的地址)
		//const unsigned char**	_pixels;
		unsigned char**	_pixels; //曹明伟修改

	private:

		//创建缓冲区;

		unsigned char ** createBuffer(int _width, int _height);

		//释放缓冲区

		bool releaseBuffer(int _height);
