...
namespace libpano
{
	...
	bool is_file(const std::string& filename)
	{
		...
		if (::_stat(filename.c_str(), &statbuf) < 0)
		{
			return false;
		}
	...
	bool is_directory(const std::string& path)
{
	...
	struct _stat statbuf;
	if (::_wstat(wpath.c_str(), &statbuf) < 0)
	{
		return false;
	}
	...
	void get_filenames_with_extension(
	...
	const std::string & extension)
{
	...
	void  get_filenames_with_absolute_path(
	...
	const std::string & extension)
{
	...
	// 使用Windows API来查找文件
	// 转换为宽字符路径
	int size_needed = MultiByteToWideChar(CP_ACP, 0, dir.c_str(), -1, NULL, 0);
	std::wstring wdir(size_needed - 1, 0);
	MultiByteToWideChar(CP_ACP, 0, dir.c_str(), -1, &wdir[0], size_needed - 1);
	
	size_needed = MultiByteToWideChar(CP_ACP, 0, extension.c_str(), -1, NULL, 0);
	std::wstring wext(size_needed - 1, 0);
	MultiByteToWideChar(CP_ACP, 0, extension.c_str(), -1, &wext[0], size_needed - 1);
	
	std::wstring wsearchPath = wdir + L"\\" + wext;
	WIN32_FIND_DATAW findData;
	HANDLE hFind = FindFirstFileW(wsearchPath.c_str(), &findData);
	if (hFind == INVALID_HANDLE_VALUE)
	{
		std::cerr << "FindFirstFileW failed for path: " << dir << "\\" << extension << std::endl;
	}
	else
	{
		do
		{
			...
			if (wname != L"." && wname != L"..")
			{
				...
				// 转换回窄字符
				int size_needed2 = WideCharToMultiByte(CP_ACP, 0, wname.c_str(), -1, NULL, 0, NULL, NULL);
				std::string name(size_needed2 - 1, 0);
				WideCharToMultiByte(CP_ACP, 0, wname.c_str(), -1, &name[0], size_needed2 - 1, NULL, NULL);
	...
	std::vector<cv::Mat> get_images_for_stitching(std::string dirname, std::string extname, bool is_divide)
	{
		...
		libpano::get_filenames_with_extension(dirname, files, extname);
		...
		{
			cv::Mat img = cv::imread(dirname + "/" + files[j]);
