...
namespace ParallelSurf
{
	...
	void Image::init(const unsigned char **iPixels, unsigned int iWidth, unsigned int iHeight)
	{
		...
		this->_pixels = const_cast<unsigned char**>(iPixels);//曹明伟注释
	...
	void Image::init(cv::Mat _Img, unsigned int iWidth, unsigned int iHeight)
	{
		...
		this->_pixels = createBuffer(_width, _height);
...
#pragma omp parallel for
	...
	Image::~Image()
	{
		...
		releaseBuffer(_height);
	}


	unsigned char ** Image::createBuffer(int _width, int _height)
	{
...
#pragma omp parallel for
	...
	bool Image::releaseBuffer(int _height)
	{
